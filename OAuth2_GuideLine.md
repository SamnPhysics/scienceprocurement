...
1. **使用者點擊登入**：我們透過 `js-auth.html` 開啟一個「彈出視窗 (Popup)」導向 Google 授權畫面，這能確保主網頁不會跳轉離開。
2. **Google 認證**：使用者同意授權後，Google 會把彈出視窗導回我們的系統，並在網址附上授權代碼 (`code`)。
3. **後端換取個資與 Token**：後端 (`Code.js`) 收到 `code` 後，與 Google 交換使用者個資，產生 Session Token 存入快取。
4. **彈出視窗傳遞 Token**：後端回傳一小段腳本給彈出視窗，該腳本透過 `postMessage` 把 Token 傳給原本的主網頁，然後自動關閉彈出視窗。
5. **前端拿通行證做事**：主網頁收到 Token 後，存入瀏覽器快取中。以後每次呼叫後端 API 都會亮出這張通行證，安全驗證身分並執行寫入資料。
...
所以在 `js-core.html` 的最上方 `<script>` 區塊，我們加入了這段魔法代碼：
```javascript
